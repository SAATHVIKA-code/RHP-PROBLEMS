...
int main() {
    ...
    // bombRow[i] = true if row i contains a bomb
    vector<bool> bombRow(H, false);

    // bombCol[j] = true if column j contains a bomb
    vector<bool> bombCol(W, false);

    // Find rows and columns containing bombs
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
            if (S[i][j] == '#') {
                bombRow[i] = true;
    ...
    // Add all safe empty cells as BFS starting points
    for (int i = 0; i < H; i++) {
        for (int j = 0; j < W; j++) {
